# 60 Hands-On Challenges Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Create 60 hands-on Databricks challenges stored in YAML format, replacing the current Python-based challenge definitions.

**Architecture:** Single `challenges.yaml` file containing all challenges grouped by category. The `challenges.py` module loads from YAML at import time and exposes the same API. All challenges require workspace interaction with deterministic answers.

**Tech Stack:** Python 3.12+, PyYAML, FastAPI, Pydantic
...
Run:
```bash
.venv/bin/python -c "import yaml; print('PyYAML version:', yaml.__version__)"
```
Expected: PyYAML version printed without error

**Step 3: Commit**

```bash
git add pyproject.toml uv.lock
git commit -m "build: add pyyaml dependency for YAML challenge storage"
```

---

## Task 2: Create challenges.yaml with SQL Challenges (sql-001 to sql-015)

**Files:**
- Create: `challenges.yaml`

**Step 1: Create YAML file with header and SQL challenges**

Create `challenges.yaml` with the following content:
...
Run:
```bash
.venv/bin/python -c "import yaml; yaml.safe_load(open('challenges.yaml')); print('YAML valid')"
```
Expected: "YAML valid"
...
```bash
git add challenges.yaml
git commit -m "feat: add SQL challenges (sql-001 to sql-015)"
```

---

## Task 3: Add AI/ML Challenges (ai_ml-001 to ai_ml-015)

**Files:**
- Modify: `challenges.yaml`

**Step 1: Append AI/ML challenges to challenges.yaml**

Add the following after the SQL challenges section:
...
Run:
```bash
.venv/bin/python -c "import yaml; data=yaml.safe_load(open('challenges.yaml')); print(f'Total challenges: {len(data[\"challenges\"])}')"
```
Expected: "Total challenges: 30"

**Step 3: Commit**

```bash
git add challenges.yaml
git commit -m "feat: add AI/ML challenges (ai_ml-001 to ai_ml-015)"
```

---

## Task 4: Add Data Engineering Challenges (de-001 to de-015)

**Files:**
- Modify: `challenges.yaml`

**Step 1: Append Data Engineering challenges to challenges.yaml**

Add the following after the AI/ML challenges section:
...
.venv/bin/python -c "import yaml; data=yaml.safe_load(open('challenges.yaml')); print(f'Total challenges: {len(data[\"challenges\"])}')"
```
Expected: "Total challenges: 45"

**Step 3: Commit**

```bash
git add challenges.yaml
git commit -m "feat: add Data Engineering challenges (de-001 to de-015)"
```

---

## Task 5: Add Platform Administration Challenges (admin-001 to admin-015)

**Files:**
- Modify: `challenges.yaml`

**Step 1: Append Platform Administration challenges to challenges.yaml**

Add the following after the Data Engineering challenges section:
...
**Step 2: Validate YAML syntax and total count**

Run:
```bash
.venv/bin/python -c "import yaml; data=yaml.safe_load(open('challenges.yaml')); print(f'Total challenges: {len(data[\"challenges\"])}')"
```
Expected: "Total challenges: 60"

**Step 3: Commit**

```bash
git add challenges.yaml
git commit -m "feat: add Platform Administration challenges (admin-001 to admin-015)"
```

---

## Task 6: Update challenges.py to Load from YAML

**Files:**
- Modify: `challenges.py`

**Step 1: Rewrite challenges.py to load from YAML**

Replace the entire contents of `challenges.py` with:
...
Run:
```bash
.venv/bin/python -c "from challenges import get_all_challenges; print(f'Loaded {len(get_all_challenges())} challenges')"
```
Expected: "Loaded 60 challenges"
...
```bash
.venv/bin/python -c "
from challenges import get_challenges_by_category
for cat in ['sql', 'ai_ml', 'data-engineering', 'admin']:
    print(f'{cat}: {len(get_challenges_by_category(cat))} challenges')
"
```
Expected:
```
sql: 15 challenges
ai_ml: 15 challenges
data-engineering: 15 challenges
admin: 15 challenges
```
...
git commit -m "refactor: load challenges from YAML instead of Python"
```

---

## Task 7: Verify Full Application Works
...
**Step 2: Test challenges endpoint**

Run:
```bash
curl -s http://localhost:8000/challenges | python -m json.tool | head -30
```
Expected: JSON array of challenges with correct structure

**Step 3: Test category filter**

Run:
```bash
curl -s "http://localhost:8000/challenges?category=ai_ml" | python -c "import sys,json; data=json.load(sys.stdin); print(f'AI/ML challenges: {len(data)}')"
```
Expected: "AI/ML challenges: 15"

**Step 4: Stop the server and commit final verification**
...
|------|-------------|---------------|
| 1 | Add PyYAML dependency | pyproject.toml, uv.lock |
| 2 | Create YAML with SQL challenges | challenges.yaml |
| 3 | Add AI/ML challenges | challenges.yaml |
| 4 | Add Data Engineering challenges | challenges.yaml |
| 5 | Add Admin challenges | challenges.yaml |
| 6 | Update challenges.py loader | challenges.py |
| 7 | Verify application works | (verification only) |

**Total commits:** 7
**New files:** 1 (challenges.yaml)
**Modified files:** 2 (challenges.py, pyproject.toml)
